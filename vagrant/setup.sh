...
drop_nomad_unit_file() {
  ...
  # Drop nomad server + nomad client + nomad-driver-containerd systemd unit file into /lib/systemd/system.
  cat << EOF > nomad.service
# /lib/systemd/system/nomad.service
[Unit]
Description=nomad client + nomad server + nomad-driver-containerd
